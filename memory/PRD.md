...
6. **P2 — Shared Domain Calculation Consolidation** — *in progress (Jul 2026)*.
   ...
   - **Slice 6** *(2026-07-22)* — **CLOSES Phase 6**: Transfer + Father's Firm settlement + account balance projections consolidated. `transfers._apply_transfer_to_account_balance` → thin adapter over `domain.apply_transfer_to_account_balance_paise` (schema-fixed from `from`/`to` → `from_side`/`to_side`, filter switched from `is_transfer_active` → new `is_transfer_countable_for_balance` which includes reversed originals). `transfers.ff_settlement_delta_from_transfers` → thin async adapter over new `sum_ff_ledger_delta_from_transfers_paise`. `transfers.derive_account_balance` accumulates in paise via domain (customer/purchase-payment active filters + split `sum_cashbook_income/expense_for_account_paise` + `apply_transfer_to_account_balance_paise`). Byte-equivalent to pre-Slice-6 float walk on 100 live accounts (zero drift). CI baselines: float 45→38 (−7), round 51→46 (−5), reversed_ne_true 1→0 (−1), source_ne_legacy_shim 3→2 (−1). 33 new Slice-6 tests including a drift-canary that pins the sign convention (dashboard vs party-ledger FF helpers must remain exact negatives on active-only data). Reconcile still healthy 21/21.
   - **PHASE 6 COMPLETE** — pending final reviewer sign-off. All 6 slices byte-equivalent. Ready to proceed to next backlog item (GST Report, Invoice PDF, legacy backend_test.py refresh, or Admin Data Management UI — deferred until Phase 6 closes per user direction).
